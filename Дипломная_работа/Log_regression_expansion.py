...
idx_stage = df_raw[df_raw.apply(lambda row: row.str.contains('Этап экономичес', na=False).any(), axis=1)].index[0]
...
header_stage = df_raw.iloc[idx_stage].ffill()
header_years = df_raw.iloc[idx_years].ffill()
header_quarters = df_raw.iloc[idx_quarters]

valid_cols = [col for col in range(2, df_raw.shape[1]) if header_stage[col] == 'Подъем']

records_macro = []
for col in valid_cols:
    year = int(header_years[col])
...
vars_bank = ['Н1', 'NPL', 'ROA', 'H3', 'log_assets', 'share_sys', 'g_credit', 'LDR', 'interbank', 'g_profit', 'ownership', 'default']
...
df_banks = df_banks.pivot_table(index=['bank', 'year'], values=vars_bank, aggfunc='first').reset_index()

df_merged = pd.merge(df_banks, df_macro, on=['year'], how='left')

target = 'default'
features = ['gdp_growth', 'inflation', 'key_rate', 'Н1', 'NPL', 'ROA', 'H3', 'log_assets', 'share_sys']
df_lr_model = df_merged[['bank', 'year', target] + features].copy()
...
df_lr_model = df_lr_model.drop(columns=['bank', 'year'])

df_lr_model = df_lr_model.where(pd.notnull(df_lr_model), None)

output_filename = 'DATA_FOR_LOG_REGRESSION_EXPANSION.csv'
