...
valid_cols = [
    col for col in range(2, df_raw.shape[1])
    if header_stage[col] == 'Пик'
]
...
banks_def_2009 = df_lr_model[
    (df_lr_model['year'] == 2009) & (df_lr_model['default'] == 1)
]['bank'].unique()
df_lr_model.loc[
    (df_lr_model['bank'].isin(banks_def_2009)) & (df_lr_model['year'] == 2008),
    'default'
] = 1
df_lr_model = df_lr_model[
    ~((df_lr_model['bank'].isin(banks_def_2009)) & (df_lr_model['year'] >= 2009))
]
...
output_filename = 'DATA_FOR_LOG_REGRESSION_PEAK.csv'
